  ...
  void UpdateCurrentAIState()
  {
    ...
    switch (aiState)
    {
      case AIState.Patrol:
        ...
        break;
      case AIState.Follow:
        m_EnemyController.SetNavDestination(Player.position);
        m_EnemyController.OrientTowards(Player.position);
        break;
      case AIState.Attack:
        if (Vector3.Distance(Player.position, m_EnemyController.m_DetectionModule.detectionSourcePoint.position)
        ...
        m_EnemyController.OrientTowards(Player.position);
        // m_EnemyController.TryAtack((Player.position - m_EnemyController.GetCurrentWeapon().weaponRoot.transform.position).normalized);
        break;
    }
  }
